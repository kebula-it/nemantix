...
class NemantixOperationException(NemantixRuntimeException):
    def __init__(self, operand: Any | tuple, operation_name: str, statement=None,
                 script=None):
        ...
        if is_binary:
            ...
            super().__init__(f'Unsupported binary operation "{operation_name}" between "{a}" '
                             f'({type(a).__name__}) and "{b}" ({type(b).__name__})!',
                             statement=statement, script=script)
        else:
            a = operand

            super().__init__(f'Unsupported unary operation "{operation_name}" on "{a}"'
                             f' ({type(a).__name__})!',
                             statement=statement, script=script)


class NemantixImportException(NemantixRuntimeException):
    def __init__(self, action_or_tool: str, deliberate_name: str | None = None,
                 action_import=False, statement=None, script=None):
        if action_import:
