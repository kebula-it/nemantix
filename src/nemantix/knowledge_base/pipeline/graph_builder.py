...
class GraphBuilder:
    ...
    @staticmethod
    def build_from_hierarchy(document_id: str, schema: DocumentHierarchyModel) -> nx.DiGraph:
        """
        ...
        graph.add_node(doc_node_id, type="document", label=schema.title,
                       doc_type=schema.document_type, coordinates={})

        # Recursive function for navigating the graph
        def _add_nodes_recursively(node: NodeModel, parent_id: str, previous_sibling_id: str = None):
            """
            ...
            if previous_sibling_id:
                graph.add_edge(previous_sibling_id, current_node_id, etype="NEXT_SIBLING")

            # Recursion through all children of the current node
            prev_child_id = None
            for child in node.children:
                _add_nodes_recursively(child, parent_id=current_node_id, previous_sibling_id=prev_child_id)
        ...
        for root_node in schema.nodes:
            _add_nodes_recursively(root_node, parent_id=doc_node_id, previous_sibling_id=prev_root_id)
            prev_root_id = root_node.node_id

        logger.debug("Successfully built graph for document %s with %d nodes and %d edges.",
                     document_id, graph.number_of_nodes(), graph.number_of_edges())
