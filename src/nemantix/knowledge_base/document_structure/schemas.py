...
class NodeModel(BaseModel):
    ...
    kind: str = Field(
        description="The structural type of the node (e.g., chapter, section)."
    )
...
class DocumentHierarchyModel(BaseModel):
    ...
    document_type: str = Field(
        default="document", description="The semantic category of the document."
    )
...
class MetadataItem(BaseModel):
    ...
    key: str = Field(
        description="The category of the metadata (e.g., 'Location', 'Date', 'Entity', 'Topic')"
    )
...
class SegmentEnrichment(BaseModel):
    ...
    summary: str = Field(
        description="A concise 1-2 sentence summary of the segment content."
    )
    metadata: List[MetadataItem] = Field(
        description="List of extracted metadata items."
    )
