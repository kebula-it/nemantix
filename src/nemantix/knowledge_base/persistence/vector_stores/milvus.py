...
class MilvusVectorStore(VectorStore):
    ...
    def search(
        ...
        fields_to_return = (
            output_fields
            if output_fields
            else ["base_node_id", "hierarchy", "text", "doc_id", "item_type"]
        )
        ...
        for hit in results[0]:
            parsed_results.append(
                {"id": hit["id"], "score": hit["distance"], "metadata": hit["entity"]}
            )

        return parsed_results

    def delete(
        self, ids: Optional[List[int]] = None, filter_expr: Optional[str] = None
    ) -> Dict[str, Any]:
    ...
    def delete_collection(self, collection_name: str) -> bool:
        ...
        try:
            ...
            logger.error(
                "Milvus error while deleting collection '%s': %s", collection_name, e
            )
            return False
