...
class TestCompileRegister:
    def test_register_returns_parser(self) -> None:
        p = argparse.ArgumentParser()
        subs = p.add_subparsers()
        sub_p = cmd_compile.register(subs)
        assert isinstance(sub_p, argparse.ArgumentParser)

    def test_register_sets_handler(self) -> None:
        p = argparse.ArgumentParser()
        subs = p.add_subparsers()
        cmd_compile.register(subs)
        args = p.parse_args(["compile", "a.nxs"])
        assert args.handler is cmd_compile.handle


class TestCompileHandle:
    def _args(
        self,
    ...
    def test_no_paths_returns_zero(self) -> None:
        assert cmd_compile.handle(self._args()) == 0

    @patch("nemantix.cli.compile.Expertise")
    def test_calls_from_local_scripts_and_build(self, mock_cls: MagicMock) -> None:
        ...
        rc = cmd_compile.handle(self._args(paths=["a.nxs"]))
    ...
    def test_output_forwarded_as_export_location(self, mock_cls: MagicMock) -> None:
        ...
        cmd_compile.handle(self._args(paths=["a.nxs"], output="build/"))
