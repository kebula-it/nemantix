...
class Observer(Observable, Storable):
    ...
    def stop_hardware_tracking(self, _: Event | None = None):
        ...
        if hasattr(self.process, 'io_counters') and self._baseline_io:
            ...
            self.hardware.io_read_count = current_io.read_count - self._baseline_io.read_count
            self.hardware.io_write_count = current_io.write_count - self._baseline_io.write_count
    ...
    def print(self, line_size=50):
        ...
        print(f"  ├─ Disk I/O:      {self.hardware.io_read_count} Reads | "
              f"{self.hardware.io_write_count} Writes")
        print(f"  └─ Network:       {self.hardware.network_kb_recv:.2f} KB Down | "
              f"{self.hardware.network_kb_sent:.2f} KB Up")

        print("\n🤖 AGENT METRICS")
        print(f"  ├─ User Requests: {self.agent.user_requests}")

        llm_calls = sum(v['internal'] + v['external']
                        for v in self.agent.llm_calls.values())
        ...
        for tool, count in sorted(self.agent.tool_frequencies.items(),
                                  key=lambda x: x[1], reverse=True):
