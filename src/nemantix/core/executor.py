...
class Executor:
    ...
    class PhaseEvent:
        def __init__(self, executor: 'Executor', phase: str, deliberate: nmx_nodes.Deliberate):
            self.executor = executor
            self.phase = phase
            self.deliberate = deliberate

        def __enter__(self):
            self.executor._emit_executor_event(EventType.EXECUTOR_PHASE_START,
                                               phase=self.phase,
                                               deliberate=self.deliberate.name)

        def __exit__(self, *args):
            self.executor._emit_executor_event(EventType.EXECUTOR_PHASE_END,
                                               phase=self.phase,
                                               deliberate=self.deliberate.name)
    ...
    def __init__(self, expertise: Expertise, llm: AbstractLLMProxy,
        ...
        if external_vars is None:
            ...
            logger.warning(f'Provided "external_vars" are ignored because not a dict but a "{type(external_vars)}"!')
            external_vars = {}
    ...
    def execute(self, user_request: str, **__):
        ...
        self._emit_executor_event(EventType.EXECUTOR_PHASE_END, phase='parse_request',
                                  deliberate=deliberate.name, uncoded=uncoded)
        ...
        if plan is not None:
            ...
            qualifier = deliberate.qualifier[1] if deliberate.qualifier is not None else None
        ...
        if not plan or qualifier != PlanQualifierEnum.FROZEN:
            ...
            with self.PhaseEvent(self, phase='code_deliberate', deliberate=deliberate):
                required_scripts = [self.expertise.script_by_loc[s] for s in
                                    self.expertise.requires_map[script.get_location()]]

                new_content = self.expertise.coder.code_deliberate(coding_level=CodeOperationEnum.COMPLETE,
                                                                   deliberate_name=deliberate.name,
                                                                   script=script, required_scripts=required_scripts,
                                                                   user_request=user_request)
                # update script in expertise (with coded deliberate)
                script = self.expertise.update_script_content(script.get_location(), deliberate, new_content)

        # check for not frozen actions that are required by this deliberate, and update all affected scripts
        modified = self.uncoded_actions_runtime_coding(deliberate=deliberate, script=script, request=user_request)

        if modified:
            output_dir = script.source_manager.get_default_export_location()
            new_filename = script.source_manager.get_file_name(script.get_location()) + "_runtime.nxc"
        ...
        return self.interpreter.interpret_coded_request(script, request_deliberate=deliberate, user_inputs=inputs)

    def execute_uncoded_request(self, user_request: str, deliberate: nmx_nodes.Deliberate,
                                script: Script):
        with self.PhaseEvent(self, phase='parse_inputs', deliberate=deliberate):
            user_inputs = self._parse_user_inputs(request=user_request, script=script,
                                                  deliberate=deliberate)

        return self.interpreter.interpret(deliberate=deliberate, user_inputs=user_inputs)

    def get_agent_state(self) -> Struct:
        return self.interpreter.agent_state

    def uncoded_actions_runtime_coding(self, deliberate: nmx_nodes.Deliberate,
                                       script: Script, request: str):
        ...
        def visit_stmt(stmt):
            ...
            if isinstance(stmt, nmx_nodes.DoStatement):
                ...
                if (callable_type == nmx_nodes.CallableTypeEnum.ACTION or
                    ...
                    elif f'{deliberate.name}.{stmt.name}' in self.expertise.private_action_to_script_loc:
                    ...
                    if action_qualifier is None or action_qualifier[1] != PlanQualifierEnum.FROZEN:
                        logger.info(
    ...
    def parse_coded_request(self, request: str) -> tuple[Script, nmx_nodes.Deliberate, nmx_nodes.Expression | None]:
        # temp script
        coded_request = self._CODED_TEMPLATE.format(request)
        logger.debug(coded_request)

        temp_script = Script(location=self._TEMP_PATH, source_manager=LocalSourceManager(),
                             content=coded_request)
        temp_script.parse()
        logger.debug(f'Deliberates: {str([v.name for v in temp_script.deliberates.values()])},'
        ...
        if len(statements) > 1:
            raise NemantixParserException(f'Only one statement allowed in coded request: '
        ...
        if node_name not in self.expertise.deliberate_to_script_loc:
            raise NemantixRuntimeException(f'Coded request: deliberate "{node_name}" does not exist!')
    ...
    def _parse_user_inputs(self, request: str, deliberate: nmx_nodes.Deliberate, script: Script,
                           max_attempts=6) -> nmx_nodes.Collection | None:
        correction = ''
        ...
        action_text = str({"inputs": action_info["ins"], "outputs": action_info["outs"]})
            ...
            try:
                ...
                if isinstance(inputs, dict):
                    for possible_key in ['inputs', 'parameters', 'data', 'args']:
                        if possible_key in inputs and isinstance(inputs[possible_key], (list, tuple)):
                ...
                for input_spec in inputs:
                    ...
                    if len(missing_fields) > 0:
                        raise NemantixRuntimeException(f'Missing fields: "{missing_fields}"')

                return self._inputs_from_request(required_inputs=inputs)

            except (json.JSONDecodeError, RuntimeError, LarkError, NemantixException) as e:
                logger.warning(f'[{e.__class__.__name__}]: {e}')
                correction = (f'Your response was faulty. Adjust to solve the error "{e}". '
    ...
    @classmethod
    def _inputs_from_request(cls, required_inputs: list[dict]) -> nmx_nodes.Collection | None:
        inputs = []
        ...
        for input_spec in required_inputs:
            ...
            if not name.isidentifier():
                raise NemantixRuntimeException(f'"{name}" is not a valid variable name!')
            ...
            if raw_kind is None:
                if value is None:
                    kind = 'none'
                elif isinstance(value, bool):  # Must check before int (bool is a subclass of int)
                    kind = 'bool'
            ...
            elif kind in ['int', 'integer', 'float', 'double', 'numeric',
                          'number', 'num']:
                stmt = f'[[{name}] = {value}]'
        ...
        collection = nmx_nodes.Collection(value=inputs, meta=meta,
                                          inferred_type=nmx_nodes.VariableTypeEnum.LIST)
        return collection
    ...
    def _promote_fallback(self, request: str, fallback_name: str) -> str:
        ...
        script = self.expertise.update_script_content(
            script_loc, original_node=script.deliberates[new_name],
        ...
        self.uncoded_actions_runtime_coding(deliberate=new_deliberate, script=script,
                                            request=request)
    ...
    def _generate_deliberate_identity(self, request: str) -> IdentitySchema:
        existing_names = sorted(set(self.expertise.deliberate_to_script_loc.keys())
    ...
    def _resolve_unique_deliberate_name(self, proposed: str, fallback_name: str) -> str:
        ...
        """
        existing = set(self.expertise.deliberate_to_script_loc.keys()) | self.expertise.fallback_names
    ...
    def _select_deliberate_name(self, request: str, prompt_correction='') -> SelectionSchema:
        ...
        prompt = DELIBERATE_SELECTION_PROMPT.format(deliberate_sem_map, request, prompt_correction)
    ...
    def _emit_request(user_request: str):
        ...
        event = Event(type=EventType.USER_REQUEST, lines=(0, 0),
                      scope='executor', script=None, statement='',
