...
def test_live_text_generation(live_google_proxy):
    """
    Tests basic text generation with the live API.
    """
    prompt = "Reply with just 'ok'."
    response = live_google_proxy.invoke(prompt)
...
@pytest.mark.integration
def test_live_function_call(live_google_proxy):
    """
    Tests the full function calling flow with the live API.
    """
    # Bind the toolset using the new signature
    live_google_proxy.bind_tools(WeatherToolset, ["get_current_weather"])
    ...
    live_google_proxy.unbind_tools()


@pytest.mark.integration
def test_live_streaming_generation(live_google_proxy):
    """
    Tests the full function calling flow with the live API.
    """
    ...
    text_response = ""
    for chunk in live_google_proxy.stream(prompt):
