...
class AzureOpenAILLMProxy(AbstractLLMProxy):
    ...
    def _build_parameter_schema(parameter: inspect.Parameter) -> Dict[str, Any]:
        ...
        try:
            # TypeAdapter perfectly maps primitives, lists, dicts, and Pydantic models to JSON Schema
            schema = TypeAdapter(ann_type).json_schema()

            # Strip out top-level metadata titles generated by Pydantic to save LLM context window tokens
        ...
        try:
            ...
            for name in tool_names:
                ...
                for p_name, param in info["parameters"].items():
                    # Generate the full JSON Schema object for the type
