...
class BaseDebugger(Observable):
    ...
    def on_line(self, event: Event) -> None:
        if self._skip_all:
            return
        ...
        if self._step_next and len(self.call_stack) <= self._step_depth:
            ...
            self._stop(event, StopReason.STEP)
    ...
    def on_breakpoint(self, event: Event) -> None:
        self._stop(event, StopReason.BREAKPOINT)

    def on_error(self, event: Event) -> None:
        self._stop(event, StopReason.EXCEPTION)
    ...
    def _stop(self, event: Event, reason: StopReason) -> None:
        """Block execution until the user/IDE issues a run-control command."""
    ...
    def _get_inline_stmt_parser():
        ...
        from nemantix.core.parser import AstTransformer, _get_stmt_parser
    ...
    def _eval(self, raw: str, interpreter) -> object | None:
        if raw.startswith("do "):
            stmt = self._get_inline_stmt_parser()(raw)
            interpreter.interpret_do_statement(stmt)
            return None
        expr = self._get_inline_parser()(raw)
        return interpreter.interpret_expression(expr)
...
class Debugger(BaseDebugger):
    ...
    def on_error(self, event: Event) -> None:
        ...
        super().on_error(event)  # → _stop(event, StopReason.EXCEPTION)

    def _stop(self, event: Event, reason: StopReason) -> None:  # noqa: ARG002
        """Implements the REPL loop"""
        if self._skip_all:
            return
        ...
        while True:
            ...
            if command in ["l", "list"]:
                if len(args) == 0:
                    ...
                    print(
                        self._list_lines(event, first=int(args[0]), last=int(args[1]))
                    )
                continue
            ...
            if command in ["e", "eval"]:
                if len(args) == 0:
                    ...
                    raw_eval = " ".join(args)
                    try:
                        result = self._eval(raw_eval, interpreter)
    ...
    @staticmethod
    def _list_lines(
        event: Event, center: int = None, first: int = None, last: int = None, context=5
    ) -> str:
